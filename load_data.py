...
class TinyDataset(Dataset):
    def __init__(self, root, opt, pil_transform = None, transform = None, tensor_transform = None):
        self.root    = root
        self.images  = sorted(glob.glob(self.root + opt + 'Masks/Large/'+ '*.jpg'))
        self.targets = sorted(glob.glob(self.root + opt + 'Images/'+ '*.jpg'))
...
class ClassificationDataset(Dataset):
    def __init__(self, root, opt, label_file, pil_transform = None, transform = None, tensor_transform = None):
        self.root= root
        self.images = sorted(glob.glob(os.path.join(self.root + opt + 'Images/' +'*.jpg')))
        f = np.load(self.root + opt + label_file)
        self.labels = f
        self.pil_transform = pil_transform
        self.transform     = transform
        self.tensor_transform = tensor_transform
